class Calculator {
  ...
  compute() {
    ...
    switch (this.operation) {
      case '+':
        computation = +(prev + current).toFixed(this.maxFixLength);
        break;
      case '-':
        computation = +(prev - current).toFixed(this.maxFixLength);
        break;
      case '*':
        computation = +(prev * current).toFixed(2 * this.maxFixLength);
        break;
      case '÷':
  ...
  getDisplayNumber(number) {
    ...
    if (this.maxFixLength < stringNumber.length - 2) {
      this.maxFixLength = stringNumber.length - 2;
    }
